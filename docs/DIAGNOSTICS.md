...
| `The body` | the angle between each step and the hips, by errand: `side` and `back` are the shuffles and backpedals of a body held off its run (`SimPlayer.look_target`), which a position trace cannot see. An errand mostly held is an errand paying the strafe cap; `the feint, first test failed` under `The small acts` is the same question for the one act that turns the body without the ball |
...
| `bodies thrown at them` | under `Shots by distance`: the lunges, their mean chance and how many got there, and where the nearest body in front of each strike stood. A low block share reads two ways -- nobody in front, or a man in front who never gets there -- and only this line tells them apart; it found the first |
...
| `Where passes are played from` | *where* on the pitch the passing happens — a 6 x 5 grid per side in its own frame, passes with completion and touches per cell, and the same by role. Every other pass count is blind to it: a side that plays everything from the centre circle and one that works the flanks have the same totals. It found 3% of passes starting in the wide fifths against a winger standing there two samples in three |
| `and what the ball to him was priced at` | under `Offering for the ball`: what the passer's softmax saw in the ball to a man on each kind of run — `success`, its four parts, `gain`, `loss`, `bias`, the gap to the winner, how far he still had to run and how near his marker stood. An offer that is on the list and never played is read here for the term that beat it, and it named `space` for the wide outlet |
