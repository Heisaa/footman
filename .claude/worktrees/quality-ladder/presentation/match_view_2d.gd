...
func _start_match(seed_value: int) -> void:
	...
	_accumulator = 0.0
	_paused = false
	print("kick-off: seed %d" % seed_value)
...
func _draw_hud() -> void:
	...
	var header := "%s %d  %d - %d   %02d:%02d  %s   x%.0f%s" % [
		"SEED", match_seed, _curr.score[0], _curr.score[1], minute, second, period, _speed,
		"  PAUSED" if _paused else "",
	]
	if _match.finished:
		header += "   —  N for the next match"
	...
	draw_string(
		...
		"space pause   1/2/3/4 speed   V value grid   N next match   R again   Esc quit",
