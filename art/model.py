...
def brows(head, head_r, depth, stand, slots, pivot, head_pivot):
    ...
    for side in ("L", "R"):
        # A **slab**, not a blob, and `M.strip` has the argument: a blob's
        # widest section exists at one depth only, so the ends of a long one
        # taper to an edge and read as pointed however square the section is.
        # Raising `power` to 8 did nothing about it, which is what said the
        # shape was wrong rather than the number.
        bar = M.strip((0.0, 0.0, 0.0), (unit, unit, unit), 20, "hair",
                      power=8.0, name="Brow" + side)
