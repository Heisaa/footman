...
This document specifies *what* to build and *why*. It deliberately contains no code and no
language-specific structure — the models, constants and criteria described here are the design,
and they hold regardless of how they are expressed.
...
simulation is deep because depth is what makes a match surprising, legible and worth watching, not
because accuracy is the target. Where the two conflict, entertainment wins: matches should have more
goals and a higher tempo than real football, and that is a feature rather than a drift to be
corrected. Realism is the *texture* — a ball that behaves like a ball, a pass that can be
misweighted, a defender who can be beaten — and the texture is what makes the entertainment feel
earned rather than arbitrary. Fidelity is in service of the game, never the other way round.

The one thing realism still decides is the floor: the sanity ranges of §11 exist so that "more
entertaining" never becomes "not recognisably football". Above that floor, the tuning targets are
set by what is fun to watch.
...
net along with the ball and no panel ever treated that as fantasy; the deadpan is the part to copy,
not the exaggeration on its own. This does not contradict the paragraph above. The *simulation*
stays honest to the sanity ranges; the comic register lives in the characters, the naming, the body
language and the copy. It is pillar 3 pointed at people rather than at objects, and §9.7 says what
...
| Module | Owns |
|---|---|
| **Ball** | Ball state, integrator, ground interaction, trajectory forecast |
| **Player** | Locomotion, stamina, attributes, separation |
| **Touch** | All ball contact — dribble, pass, shot, first touch, clearance, header |
| **Value field** | Pitch control and expected-threat evaluation |
| **Decision** | Candidate action generation and expected-value scoring |
| **Movement** | Off-ball positioning, pressing, defensive assignment |
| **Keeper** | Goalkeeper positioning and shot response |
| **Set piece** | Restart state machine and routines |
| **Referee** | Offside, fouls, cards, advantage, added time |
| **Tactics** | Translating a tactical plan into decision-layer modifiers |
| **Telemetry** | The match event log and positional trace |
| **Match** | Fixed-step tick loop, phase of play, orchestration |
| **World** | Season, squad, morale, board, run state — everything outside a match |
| **Presentation** | Reads snapshots, draws them, handles all UI. Depends on everything; nothing depends on it. |
...
- Fixed timestep of 1/60 s. **Never use frame delta anywhere in the sim.**
- All randomness from a single seeded generator instance owned by the match, advanced only inside
  the tick loop. Never use global/engine-level random calls. Rendering must never consume from it.
- Iterate collections in a stable, defined order. Index players by a fixed id.
- Do not read the system clock, the frame counter, or input state from within the sim.

Maintain a determinism test that runs one seed twice and asserts identical event logs.
...
2. **Reduced fidelity** — same logic, every decision cadence strided: off-ball targets, chase
   ...
   smoke run of §11.1, which is the check that has to be cheap enough to run on every change.
...
the sim is retuned. This keeps the league consistent with the
match engine without paying for it, and it is the right design regardless of implementation
language.
...
| Drag coefficient | 0.25 (turbulent regime; ignore the drag crisis for v1) |
...
- Once rolling, apply a rolling resistance deceleration of about 1.0 m/s² on dry mown grass — a
  coefficient of rolling resistance near 0.10, which is where a football on turf sits. Grass length
  and wetness are separate effects that move in opposite directions and compose: long grass raises
  it to about 1.5, and a wet surface is greasy, so it *multiplies* by about 0.85 and the ball runs
  on faster. Wet long grass is still slower than a dry mown pitch.
...
ball has a hole in it, and the hole is large. A dribbler pushes the ball two to four metres ahead
and runs onto it, so the ball is usually out in front of him and he is only within control range of
it at the instant he touches it. A defender tracking him from a metre behind is three to five metres
from the ball and can therefore never contest: no shoulder-to-shoulder, no tackle from behind, no
nicking it away as the carrier takes a touch. His only route to the ball is to get goal-side and
intercept a pass. The observable symptoms are a carrier who runs unopposed until he chooses to pass,
carries of ten touches and more, tackle counts a third of the real figure, and turnovers that are
almost entirely clean interceptions.
...
- **Technical** — first touch, passing, technique, finishing, power, heading, tackling, dribbling, crossing
- **Mental** — decisions, awareness, positioning, composure, work rate, aggression, teamwork
- **Keeper** — reflexes, handling, command, distribution
...
**Sanity ranges — the pass/fail criterion.** Wide, structural, and never tuned toward. Any engine
...
twenty goals a match, nobody ever shooting — all trip a wire. These decide whether a change is good
or bad.
...
ten-minute gate gets run at the end of a day's work; a forty-second one gets run after every idea,
and catching a broken change in forty seconds is worth more than measuring an unbroken one to three
decimal places.
...
- **Smoke — six matches of twelve minutes, reduced fidelity.** Under a minute. Judged on the sanity
  ranges. This is the loop: run it on every change that touches the sim.
...
hundred matches can tell a passing engine from a failing one on that metric. It converges by 200
and it does not converge earlier because a faster gate would prefer it did.
...
The same reasoning applies to the test suite, which is the check run most often of all. A test that
simulates a match is paying two seconds per minute of match clock, so the suite is written to
simulate as few matches as it can and to get several assertions out of each one: the whole-match
invariants — reaching full time, the ball staying in play, no swarming, shape held, possession
alternating — are five checks over one stepped match, not five matches. Coverage across seeds is
what the smoke and gate runs are for; the suite's job is to be fast enough that there is no excuse
for skipping it. Target: under two minutes.
...
and a metric leaving one of them is a bug however good it looks. The **tuning targets** above that
floor are design decisions, and moving one is a conversation rather than a defect.
...
statistical bands above as regression gates, and the determinism test. The first and third are
absolute and always have been; the second is the two-layer arrangement of §11.1 — sanity ranges
enforced on every run, tuning targets enforced at the freeze.

---

## 12. Open questions

- `[DECIDE]` Squad size and format for the prototype phases. Six-a-side is recommended through
  Phase 3, eleven-a-side from Phase 4.
- `[DECIDE]` Whether own-squad attributes ever become fully known (§6.1).
- `[DECIDE]` Run length — one season or up to three. This affects how much the training and
  cohesion layer matters.
- `[DECIDE]` Whether the player can watch at 1× at all, or whether the punctuated flow of §6.3 is
  the only viewing mode. Recommendation: both, with punctuated as the default.
- `[DECIDE]` Injury model depth — binary availability versus a fitness and risk system.
- `[DECIDE]` Whether tactical patterns are per-run unlocks, persistent meta-progression, or both.
