...
func _build_world() -> void:
	# The surface the ground mesh is cut from has to be the one the ball will be
	# integrated over, and the seed decides it.
	_env = SimRunner.env_for(match_seed)
...
func _process(delta: float) -> void:
	...
	_draw_frame(clampf(_accumulator / SimConsts.DT, 0.0, 1.0), float(stepped) * SimConsts.DT)
	_maybe_shoot()
